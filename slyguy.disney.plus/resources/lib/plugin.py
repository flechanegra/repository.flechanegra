...
def play(content_id=None, family_id=None, skip_intro=None, continue_watching=0, **kwargs):
    if KODI_VERSION > 18:
        ver_required = '2.6.0'
    else:
        ver_required = '2.4.5'
    ...
    if video['programType'] == 'episode' and settings.getBool('play_next_episode', True):
        ...
        for row in data.get('items', []):
            if row['type'] == 'DmcVideo' and row['programType'] == 'episode' and row['encodedSeriesId'] == video['encodedSeriesId']:
                item.play_next['next_path'] = plugin.url_for(play, content_id=row['contentId'])
                break
